...
ns_ls = sorted(glob.glob(noise_source_path+"/*.CH1.wav"))
...
for tag in range(trainset_num):
    ...
    npath = os.path.join(des_path, '%s_%s_%d_noisy.flac' % (file_prefix, noise_type, snr_des)) #audio
    torchaudio.save(npath, noisy, fs)
    tpath = os.path.join(des_path,'%s_%s_%d_trans.txt'%(file_prefix, noise_type, snr_des)) #text
